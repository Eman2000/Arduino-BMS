...
void setup() {                // void setup runs once upon power up
  ...
  lcd.write("Version 0.8");   //write more to the LCD
...
void safetyCheck() {
  if (cell1AverageV >= protectVal || cell2AverageV >= protectVal || cell3AverageV >= protectVal) {  //if any of the cells rise above a safe voltage or if too much current is flowing into the pack
    ...
    for (int i = 0; i < 3000; i++) {     //wait a while while reading the buttons
      readSwitch();
      if (buttonState != 0) {         //if a button is pressed
        i = 3100;                   //set i over 30000 and end the delay (acts like an interupt)
      }
      delay(10);
    }
    safetyTrip = 1;
  ...
  if (averageAmps > ampCutoff) {  //if any of the cells rise above a safe voltage or if too much current is flowing into the pack
    digitalWrite(powerIn, LOW);       //turn off the incoming power
